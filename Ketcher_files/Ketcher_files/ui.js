...
{
    if (Prototype.Browser.IE){
        ui.client_area.style.width = (Element.getWidth(ui.client_area.parentNode) - 80).toString() + 'px';
    }

    ui.client_area.style.width = (Element.getWidth(ui.client_area.parentNode)).toString() + 'px';
    //alert((Element.getWidth(ui.client_area.parentNode)).toString())
    ui.client_area.style.height = (Element.getHeight(ui.client_area.parentNode) - 10).toString() + 'px';
};
...
*/
    ...
    if (mode == null) {
        ...
        $('reaction').removeClassName('buttonSelected');
        $('rgroup').removeClassName('buttonSelected');
        this.render.current_tool = this.editor.toolFor(mode);
        this.mode_id = mode;
        if (this.mode_id.split('_')[0] == 'reaction'|| this.mode_id.split('_')[0] == 'rgroup'){
            button_id = this.mode_id.split('_')[0];
        }else
            button_id = this.mode_id;
